...
public class QueryBuilder<T> {
    ...
    private void appendJoinsAndWheres(StringBuilder sb, String str) {
        ...
        for (Join<T, ?> join : this.joins) {
            sb.append(" JOIN ").append(join.daoDestination.getTablename()).append(' ');
        ...
        while (true) {
            ...
            if (it.hasNext()) {
                Join<T, ?> next = it.next();
                if (!next.whereCollector.isEmpty()) {
                    if (!z2) {
                        sb.append(" WHERE ");
                        z2 = true;
                    } else {
                    ...
                    next.whereCollector.appendWhereClause(sb, next.tablePrefix, this.values);
                }
                z = z2;
            } else {
                return;
